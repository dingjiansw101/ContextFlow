...
CONFIG="pi0_libero_incontextv18_low_mem_finetune_sample_frames8"  # on-disk exp/ckpt name (pre-rename)
POLICY_CONFIG="ContextFlow"  # get_config lookup (renamed)
...
if ! find "assets/${ASSETS_NAME}" -maxdepth 4 -type f -name 'norm_stats*' 2>/dev/null | grep -q .; then
    echo "Missing assets/${ASSETS_NAME} norm stats; run scripts/compute_norm_stats.py --config-name ${POLICY_CONFIG} first." >&2
    exit 66
fi
