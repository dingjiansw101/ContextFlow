...
assets_base_for_config() {
    case "$1" in
        pi0_fast_incontext_prompt_action_7_state_8*) printf "%s" "$FAST_ASSETS_BASE_DIR" ;;
        *) printf "%s" "$LIBERO_ASSETS_BASE_DIR" ;;
    esac
}
...
num_workers_for_config() {
    if [ -n "$NUM_WORKERS" ]; then
        printf "%s" "$NUM_WORKERS"
        return
    fi
    case "$1" in
        pi0_libero_incontextv18_low_mem_finetune_sample_frames8*) printf "2" ;;
        *) printf "32" ;;
    esac
}
