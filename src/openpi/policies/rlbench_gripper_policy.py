...
class RLBenchGripperInputs(transforms.DataTransformFn):
    """
    ...
    def __call__(self, data: dict) -> dict:
        ...
        if "actions" in data:
            ...
            get_action = data["actions"]

            original_type = type(get_action)

            # get components of action
            pos = get_action[:, :3] 
            quat = get_action[:, 3:7]
            gripper = get_action[:, 7:]


            # convert to axis-angle (in batch)
            axis_angle = np.stack([_quat2axisangle(q) for q in quat])

            gripper = gripper.reshape(-1, 1)  # 之后再 reshape


            # axis_angle = _quat2axisangle(quat)
            # concat：pos + axis_angle + gripper
            new_action = np.concatenate([pos, axis_angle, gripper], axis=1, dtype=np.float32)


            if isinstance(get_action, np.ndarray):
                new_action = new_action
            elif torch.is_tensor(get_action):
                new_action = torch.from_numpy(new_action).to(get_action.device)
            elif isinstance(get_action, jax.Array):
                new_action = jax.numpy.asarray(new_action)
            else:
                raise TypeError(f"Unknown action type: {original_type}")

            actions = transforms.pad_to_dim(new_action, self.action_dim)
...
@dataclasses.dataclass(frozen=True)
class RLBenchGripperOutputs(transforms.DataTransformFn):
    """
    ...
    def __call__(self, data: dict) -> dict:
        # action is 8, but after quaternion to axis-angle conversion, it is only 7
