...
def make_robocasa_insertion_example() -> dict:
...
class RobocasaInsertionInputs(transforms.DataTransformFn):
    """
    ...
    def __call__(self, data: dict) -> dict:
        ...
        base_image_left = _parse_image(data["observation/image"])
        ...
        inputs = {
            "state": state,
            "image": {
                "base_0_rgb": base_image_left,
                "left_wrist_0_rgb": wrist_image, 
                "right_wrist_0_rgb": np.zeros_like(base_image_left), 
            },
            "image_mask": {
                "base_0_rgb": np.True_,
        ...
        if "actions" in data:
            ...
            if len(data["actions"].shape) == 2:
                actions = transforms.pad_to_dim(data["actions"][:,:7], self.action_dim)
            else: 
                actions = transforms.pad_to_dim(data["actions"][:7], self.action_dim)
...
@dataclasses.dataclass(frozen=True)
class RobocasaInsertionOutputs(transforms.DataTransformFn):
    """
