...
class Pi0LightConfig(_model.BaseModelConfig):
    dtype: str = "bfloat16"
    ...
    @override
    def create(self, rng: at.KeyArrayLike) -> "Pi0Light":
        return Pi0Light(self, rngs=nnx.Rngs(rng))
    ...
    def get_freeze_filter(self) -> nnx.filterlib.Filter:
        ...
        # Define regex filters for both experts
        gemma_params_filter = nnx_utils.PathRegex(".*llm.*")
        action_expert_params_filter = nnx_utils.PathRegex(".*llm.*_1.*")

        # Freeze base model if LoRA is applied to it
        if "lora" in self.paligemma_variant:
            freeze_targets.append(gemma_params_filter)
            if "lora" not in self.action_expert_variant:
                # Avoid accidentally freezing action expert if it's not LoRA
                filters.append(nnx.Not(action_expert_params_filter))
            has_lora = True
        elif "lora" in self.action_expert_variant:
            freeze_targets.append(action_expert_params_filter)
            has_lora = True
        ...
        # Exclude all LoRA parameters from being frozen
        if has_lora:
...
class Pi0Light(_model.BaseModel):
    def __init__(self, config: Pi0LightConfig, rngs: nnx.Rngs):
        ...
        #XJ: debug
        logger.info(f"[Pi0] PaliGemma config width: {paligemma_config.width}")
        logger.info(f"[Pi0] Action Expert config width: {action_expert_config.width}")

        self.expected_vlm_dim = paligemma_config.width
        # XJ: for SigLIP Ti/16
