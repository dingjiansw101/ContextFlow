...
# --- BEGIN: Replace your Attention class with this one ---
...
class Attention(nn.Module):
    ...
    def __call__(self, xs, positions, attn_mask, kv_cache):
        ...
        if 0 < Tb < T_total:
            ...
            # b-queries：P_bb @ V_b
            P_bb   = probs[:, :, :, :Tb, :Tb]                             # [B,K,G,Tb,Tb]
            y_b    = jnp.einsum("BKGTS,BSKH->BTKGH", P_bb, v[:, :Tb, ...])
            # a-queries：P_ab @ V_b^sg  +  P_aa @ V_a
            P_ab   = probs[:, :, :, Tb:, :Tb]                             # [B,K,G,Ta,Tb]
            P_aa   = probs[:, :, :, Tb:, Tb:]                             # [B,K,G,Ta,Ta]
            y_ab   = jnp.einsum("BKGTS,BSKH->BTKGH", P_ab, v_b_sg)
...
# --- END: Replace your Attention class with this one ---
