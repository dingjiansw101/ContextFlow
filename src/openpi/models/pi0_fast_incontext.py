    ...
    def inputs_spec(self, *, batch_size: int = 1) -> tuple[_model.ObservationIncontext, _model.Actions]:
        ...
        with at.disable_typechecking():
            observation_spec = _model.ObservationIncontext(
                ...
                tokenized_incontext_states=jax.ShapeDtypeStruct([batch_size, self.max_token_len], jnp.int32),
                tokenized_incontext_states_mask=jax.ShapeDtypeStruct([batch_size, self.max_token_len], bool),
                incontext_states_ar_mask=jax.ShapeDtypeStruct([batch_size, self.max_token_len], jnp.int32),
                incontext_states_loss_mask=jax.ShapeDtypeStruct([batch_size, self.max_token_len], jnp.bool_),
                tokenized_incontext_actions=jax.ShapeDtypeStruct([batch_size, self.max_token_len], jnp.int32),
                tokenized_incontext_actions_mask=jax.ShapeDtypeStruct([batch_size, self.max_token_len], bool),
                incontext_actions_ar_mask=jax.ShapeDtypeStruct([batch_size, self.max_token_len], jnp.int32),
                incontext_actions_loss_mask=jax.ShapeDtypeStruct([batch_size, self.max_token_len], jnp.bool_),
            )
...
class Pi0FASTIncontext(_model.BaseModel):
    def __init__(self, config: Pi0FASTIncontextConfig, rngs: nnx.Rngs):
        ...
        self.siglip_variant = config.siglip_variant # "So400m/14", "Ti/16", "S/32",
    ...
    def embed_inputs(
        ...
        if obs.tokenized_incontext_states is None or obs.tokenized_incontext_states_mask is None:
            raise ValueError("Missing in-context state tokens or mask")
        if obs.incontext_states_ar_mask is None:
            raise ValueError("Missing in-context state AR mask")
        state_embeddings = self.PaliGemma.llm(obs.tokenized_incontext_states, embed_only=True)
        token_embeddings.append(state_embeddings)
        input_mask.append(obs.tokenized_incontext_states_mask)
        ar_mask.append(obs.incontext_states_ar_mask)

        # 3) in-context action tokens
        if obs.tokenized_incontext_actions is None or obs.tokenized_incontext_actions_mask is None:
            raise ValueError("Missing in-context action tokens or mask")
        if obs.incontext_actions_ar_mask is None:
            raise ValueError("Missing in-context action AR mask")
        action_embeddings = self.PaliGemma.llm(obs.tokenized_incontext_actions, embed_only=True)
        token_embeddings.append(action_embeddings)
        input_mask.append(obs.tokenized_incontext_actions_mask)
        ar_mask.append(obs.incontext_actions_ar_mask)

        # 4) current observation image tokens
        ...
        # 5) prompt tokens (include current state/action chunk from FAST tokenizer)
