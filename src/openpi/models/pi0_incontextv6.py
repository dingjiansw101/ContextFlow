...
class Pi0IncontextConfigv6(_model.BaseModelConfig):
    dtype: str = "bfloat16"
    ...
    @override
    def create(self, rng: at.KeyArrayLike) -> "Pi0Incontextv6":
        return Pi0Incontextv6(self, rngs=nnx.Rngs(rng))
...
class Pi0Incontextv6(_model.BaseModel):
    def __init__(self, config: Pi0IncontextConfigv6, rngs: nnx.Rngs):
    ...
    def embed_prefix(
        ...
        for name in obs.incontext_images:
            ...
            if count_incontext_images == 0:
                ar_mask += [False] * (image_sqeuence_tokens.shape[1] - 1)
            else:
