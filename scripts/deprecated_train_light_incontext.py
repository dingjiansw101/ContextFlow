...
def summarize_nnx_model(
    ...
    output_path: str = "model_structure_light_incontext_summary.json"
):
...
def _load_weights_and_validate(
    ...
    *,
    ...
    # Step 3c: Fill in known-missing keys (e.g. image_proj) to make check_pytree_equality pass
    for k, v in flat_expected.items():
        if k.startswith("image_proj") and k not in flat_loaded:
            flat_loaded[k] = v
...
def init_train_state(
    ...
    def init(rng: at.KeyArrayLike, partial_params: at.Params | None = None) -> training_utils.TrainState:
        ...
        #     summarize_nnx_model(model, config.freeze_filter, output_path="model_structure_v12_summary.json")
