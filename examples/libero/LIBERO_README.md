...
| `assets/ContextFlow/libero/` (norm stats) | Google Drive `assets/` (archived as `ContextFlow_Plain/physical-intelligence/libero`; rename after download) | **training only** — inference loads norm stats from the checkpoint. The ContextFlow configs resolve this path via `assets_repo_override="ContextFlow"` + `AssetsConfig(asset_id="libero")` |
