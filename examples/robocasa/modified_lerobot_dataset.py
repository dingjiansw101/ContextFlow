...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # XJ: optimize Dataset.select by using lru
        self.cached_arrays = {
            key: np.array(self.hf_dataset[key])
            for key in ["actions"]
        }
        
    def _select(self, key: str, q_idx: tuple[int]) -> torch.Tensor:
        return torch.stack(self.hf_dataset.select(list(q_idx))[key])
    ...
    # XJ: optimize Datasets.select() with LRU
    def _query_hf_dataset(self, query_indices: dict[str, list[int]]) -> dict:
        return {
            key: torch.tensor(self.cached_arrays[key][q_idx])
            for key, q_idx in query_indices.items()
        }
    ...
    def __getitem__(self, idx) -> dict:
        ...
        # end = time.time()
        # print(f"__getitem__ time 1: {end - start:.2f}s")
        # start = time.time()
        ...
        if self.delta_indices is not None:
            ...
            # end2 = time.time()
            # print(f"__getitem__ time 2.1: {end2 - end:.3f}s")
            query_result = self._query_hf_dataset(query_indices)
            # end3 = time.time()
            # print(f"__getitem__ time 2.2: {end3 - end2:.3f}s")
            item = {**item, **padding}
            for key, val in query_result.items():
                item[key] = val

        # end = time.time()
        # print(f"__getitem__ time 2: {end - start:.2f}s")
        # start = time.time()
